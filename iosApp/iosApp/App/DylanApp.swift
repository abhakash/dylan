...
final class AppEnvironment {
    ...
    // MARKETING_VERSION is synced from root VERSION via tools/sync-ios-version.sh
    // (Info.plist CFBundleShortVersionString = $(MARKETING_VERSION)), so the lookup
    // below must succeed — "0.0.0" only surfaces a packaging bug, never a real version.
    ...
    private init() {
        ...
        // Lock screen / Control Center requires the app to be a remote-control event
        // receiver; without this MPNowPlayingInfoCenter updates never surface.
        ...
        // Online ⇔ path.status == .satisfied (extends the same callback, UI-side only).
    ...
    /// Row tap: offline + uncached ⇒ OFFLINE toast, otherwise play (shared ensureReady untouched).
    func playOrOffline(_ songs: [KSong], at index: Int) {
        guard !songs.isEmpty, songs.indices.contains(index) else { return }
        guard canPlay(songs[index]) else {
            toasts.show(offlineCopy)
            return
        }
        graph.submit(Intents.playNow(songs, at: index))
    }
    ...
    func canPlay(_ song: KSong) -> Bool {
        isOnline || cachedKeys.contains(song.key.token)
    }

    func isPlaying(_ song: KSong) -> Bool {
        player.current?.key.token == song.key.token && player.showsPause
    }
