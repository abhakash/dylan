...
typealias KNativeAudioOutput = NativeAudioOutput
//   check: @protocol EngineEventSink ("EngineEventSink" + -Protocol, A2)
typealias KEngineEventSink = EngineEventSink
...
// ---- events ---------------------------------------------------------------------
//   check: enum TransitionReason { AUTO, SEEK, EXPLICIT } → .auto_/.seek/.explicit_
...
extension KGraph {
    ...
    func isFavorite(_ song: KSong) async -> Bool {
        do {
            let v = try await container.favorites.isFavorite(key: song.key) as? KotlinBoolean
    ...
    func loadHighQualityPref() async -> Bool {
        do {
            let v = try await isHighQualityPref() as? KotlinBoolean
    ...
    func clearCacheNow() async -> Int64 {
        do {
            let v = try await clearCacheExcludingProtected() as? KotlinLong
