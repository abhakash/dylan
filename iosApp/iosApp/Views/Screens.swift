                ...
                if !env.home.jumpBack.isEmpty {
                    ...
                    ForEach(env.home.jumpBack, id: \.key.token) { song in
                        SongRowView(
                            ...
                            onTap: { env.playOrOffline([song], at: 0) }
                        )
                    }
                }
...
struct LibraryScreen: View {
    ...
    var body: some View {
        ScrollView {
            LazyVStack(alignment: .leading, spacing: 0) {
                ...
                if env.library.favorites.isEmpty {
                    ...
                    ForEach(Array(env.library.favorites.enumerated()), id: \.element.key.token) { idx, song in
                        SongRowView(
                            ...
                            isCached: env.isCached(song),
                            enabled: env.canPlay(song),
                            onTap: { env.playOrOffline(env.library.favorites, at: idx) },
                            onPlayNext: { env.graph.submit(Intents.playNext(song)) },
                            onAddLast: { env.graph.submit(Intents.addLast(song)) },
                ...
                if !env.library.jumpBack.isEmpty {
                    SectionTitle("Jump back in")
                    ForEach(Array(env.library.jumpBack.enumerated()), id: \.element.key.token) { _, song in
                        SongRowView(
                            song: song,
                            index: nil,
                            isPlaying: env.isPlaying(song),
    ...
    private var downloadsCard: some View {
        Button { showDownloads = true } label: {
            ...
            .background(RoundedRectangle(cornerRadius: DylanTokens.radiusMd).fill(DylanTokens.surfaceVariant))
            .contentShape(RoundedRectangle(cornerRadius: DylanTokens.radiusMd))
        }
        .buttonStyle(.plain)
    ...
    private var downloadsSubtitle: String {
        let count = env.library.downloads.count
        if count == 0 { return "Nothing saved yet" }
        return "\(count) songs \u{00B7} \(formatBytes(env.library.totalBytes))"
    }
}
...
struct DownloadsScreen: View {
    ...
    @State private var pending: KCachedSongInfo?
    ...
    var body: some View {
        ...
        .background(DylanTokens.background)
        .task { await env.library.loadDownloads(env.graph) }
        .alert("Remove download?", isPresented: Binding(
            ...
            if let p = pending {
                Text("\"\(p.song.title)\" (\(formatBytes(p.bytes))) will be deleted from storage.")
            }
        }
    }
}
            ...
            LazyVStack(alignment: .leading, spacing: 0) {
                ZStack(alignment: .bottomLeading) {
                    ...
                    VStack(alignment: .leading, spacing: DylanTokens.s8) {
                        ...
                        HStack(spacing: DylanTokens.s12) {
                            Button {
                                if !songs.isEmpty { env.playOrOffline(songs, at: 0) }
                            } label: {
                                Text("Play")
                                    .font(.dylTitleMedium)
                            ...
                            Button {
                                ...
                                if !songs.isEmpty {
                                    ...
                                    if playingHere {
                                        ...
                                        if !env.player.shuffleOn { env.graph.submit(Intents.toggleShuffle) }
                                        env.playOrOffline(songs, at: Int.random(in: 0 ..< songs.count))
                                    }
                                }
                            } label: {
                                Text("Shuffle")
                ...
                ForEach(Array(songs.enumerated()), id: \.element.key.token) { i, song in
                    SongRowView(
                        ...
                        isCached: env.isCached(song),
                        enabled: env.canPlay(song),
                        onTap: { env.playOrOffline(songs, at: i) },
                        onPlayNext: { env.graph.submit(Intents.playNext(song)) },
                        onAddLast: { env.graph.submit(Intents.addLast(song)) }
                    )
                }
