...
object Probe {
    ...
    suspend fun run(
        ...
        check("P5", "M0", "catalog cacheability over ${sample.size} sampled songs") {
            val nonCacheable = sample.count { !it.cacheable }
            val pct = if (sample.isEmpty()) 100.0 else 100.0 * nonCacheable / sample.size
            require(pct <= 5.0, "non-cacheable share $pct%% exceeds 5%% product gate ($nonCacheable/${sample.size})")
            "date=2026-08-22 nonCacheable=$nonCacheable/${sample.size} (${"%.1f".format(pct)}%)"
        }
